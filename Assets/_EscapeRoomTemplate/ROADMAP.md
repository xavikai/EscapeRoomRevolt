...
| ID | Àrea | Implementació pendent | Criteri d'acceptació |
|---|---|---|---|
| ARC-001 | Assemblies | `Core` ja separat (vegeu `P0-001`). Les citacions `Systems`/`Player`→`UI` (`UIManager.Instance`/`UIToolkitMenuController.Instance`) ja estan desacoblades amb `EventBus` (fase 2b). Pendent: `Systems`, `Player`, `UI`, `Editor` i XR opcional encara viuen a `Assembly-CSharp` — cal crear els `asmdef` pròpiament i resoldre l'entrellaçat restant de `Systems` cap a tipus concrets de `Player` (usar `IPlayerPlatformAdapter` en lloc de `PlayerMovement`/`VRComfortController` directes en ~6 fitxers de `Systems.Survival`) abans de poder separar-los sense trencar la compilació. | Dependències unidireccionals, XR opcional i temps de recompilació reduït; tests poden referenciar runtime sense internals accidentals. |
