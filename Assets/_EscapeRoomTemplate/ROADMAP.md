...
Data de l'última auditoria: 13 d'agost de 2026
Projecte auditat: Unity `6000.4.9f1`, URP `17.4.0`, Input System `1.20.0`, XRI `3.3.0`  
Perfil actiu actual: `EscapeRoom`
...
| Preparació comercial | Parcial | 12 tests EditMode i 14 PlayMode passant, assemblies separades i demo Escape Room funcionalment tancada; falten més EditMode, QA humà i l'inventari de llicències. |

## 2. Evidència recollida durant l'auditoria

### Mida i estructura

- Sis escenes habilitades: `Intro`, `MainMenu`, `ShowcaseMuseum`, `LockedOffice`, `SurvivalHorrorDemo` i `VRTemplate`.
- Assemblies pròpies per `Core`, `Player`, `Systems`, `UI` i tests de Core.
...
| Escena | GameObjects | Missing scripts/prefabs | Canvas | UIDocument | Observació |
|---|---:|---:|---:|---:|---|
| Intro | 3 | 0 | 0 | 0 | Seqüència opcional abans del menú |
| MainMenu | 3 | 0 | 0 | 1 | Menú complet i localització ES/EN |
| ShowcaseMuseum | 321 | 0 | 0 | 2 | 82 saveables, IDs únics, 8/8 puzles resolts |
| LockedOffice | 93 | 0 | 0 | 2 | Dos panells de codi amb definició |
| SurvivalHorrorDemo | 96 | 0 | 0 | 2 | Vertical slice; conserva placeholders visuals |
| VRTemplate | 76 | 0 | 0 | 2 | Estructura vàlida; pendent hardware |
...
Unity Test Framework executa 12 tests EditMode reals (`EventBus` i `LocalizationCatalog`) i 14 PlayMode reals. PlayMode cobreix codi, seqüència, llançament, canonades, lliscant, grups de puzles encadenats —fills sempre visibles, ordre lliure o obligatori—, rodets amb focus i pas bidireccional compartit entre botons PC/VR, autoria dinàmica de 2–8 rodes, perill mòbil en qualsevol direcció, temporitzador HUD independent, les sis opcions principals del menú i un round-trip Save/Load en memòria. Tots 26 passen; l'objectiu de 10 PlayMode està superat i falta arribar a 20 EditMode.
...
- Puzles de codi, seqüència, estat, socket, llançament, col·locació, lliscant, multi-fase i canonades; la presentació de melodia reutilitza seqüència.
...
A més dels quatre tipus de puzle existents (codi, seqüència, estat i socket), `MultiStagePuzzle` actua com a coordinador d'una llista arbitrària de `PuzzleController`: tots els fills romanen visibles, es poden exigir en ordre lliure o obligatori i el payoff final només s'activa quan tots estan resolts. En mode ordenat, els fills futurs es poden mantenir visibles però temporalment no interactuables.
...
Cinquè tipus de puzle: `PlacementPuzzle`, col·locar peces a sockets (`Connect(pieceId, socketId)`/`Disconnect(pieceId)`), amb exclusivitat de socket (col·locar-hi una peça en treu qualsevol altra que hi hagués), verificació automàtica en omplir totes les connexions i Save/Load de l'estat de col·locació. Els kits de dials/safe i símbols ja estaven coberts per `CodePanelPuzzle` (accepta qualsevol token de text, no només xifres).
...
`SaveManager` ara guarda un `RunSeed` estable per partida (es renova a `StartNewGame`, es restaura en carregar). `PuzzleController.ResolveVariantSeed()` el combina amb el `SaveId` de cada puzle perquè cadascun tingui la seva pròpia variant determinista. Quatre dels set tipus ja ho aprofiten amb un toggle opcional (desactivat per defecte, sense canviar res del contingut ja autoria't): `CodePanelPuzzle` (`_randomizeCode`, codi diferent cada partida), `SequencePuzzle` (`_randomizeOrder`, mateixos passos en ordre barrejat), `PlacementPuzzle` (`_randomizeMapping`, mateixes peces/sockets amb aparellament barrejat) i `PipePuzzle` (`_randomizeRotations`, mateixes peces amb rotació inicial barrejada). Tots desen la variant triada dins l'estat propi del puzle perquè carregar una partida mai la torni a sortejar per sota d'un puzle ja resolt o d'una pista ja llegida.
...
1. Cobertura parcial: 12 EditMode i 14 PlayMode (objectiu: 20 + 10); la part PlayMode ja supera el mínim.
2. Falta confirmar llicències dels àudios i tancar `ThirdPartyNotices.md`.
3. La demo Escape Room ja té definicions per Throw/Melody i una resposta persistent de porta + balisa al Pipe Puzzle; queda el QA humà final i el slot visual de Placement.
4. VR és funcional en simulador, però no s'ha validat en hardware real.
5. La localització és parcial: menú principal/pausa ES/EN, però HUD, inventari, prompts i missatges encara contenen literals.
6. No hi ha migrador per versió del format global de `SaveGameData`; és un problema teòric fins que existeixi una segona versió real. L'escriptura ja és atòmica amb backup recuperable.
...
- Encara hi ha wrappers legacy a `UI/PC`; cal decidir quins es mantenen només per compatibilitat abans d'empaquetar.
- El desacoblament principal entre Player, Systems i UI ja passa per `EventBus` i `GameplayBlockState`.
...
- Els `asmdef` de Core, Player, Systems i UI ja eliminen el bloqueig estructural; falta ampliar tests sobre aquestes assemblies.
- Existeix `LocalizationService`, però la migració de literals a catàleg encara és parcial.
...
- Els validadors cobreixen assets, IDs, cicles d'objectius, receptes i contracte UXML, però encara no detecten un puzle resoluble sense cap feedback `OnSolved`.
- L'escriptura atòmica i la recuperació `.bak` estan resoltes; falten preflight complet de build i migracions generals de saves.
- Les captures de ranura continuen sent asíncrones i necessiten una prova d'estrès específica.
- Els inspectors de Sliding han millorat, però encara falta una experiència guiada homogènia per tots els puzles.
...
Actualització 2026-08-13: qualsevol recompte antic dins l'historial extens de `P0-001` queda substituït pel resultat verificat actual, **12/12 EditMode i 14/14 PlayMode**. El mínim PlayMode ja està superat; només resta ampliar EditMode fins a 20.
...
| P0-006 | Documentació | Actualització completa el 2026-08-13: `README`, `CHANGELOG`, notes de release, `DOCUMENTACIO_COMPLETA.md`, `UserManual.md`, `PROGRAMMING_GUIDE.md`, checklist comercial, auditoria i roadmap alineats amb les escenes PC/VR, els grups de puzles visibles i els controls ▲/▼ dels rodets. Manteniment continu: actualitzar-los quan canviï una demo o un criteri de publicació. | README, UserManual, Programming Guide, documentació completa i roadmap descriuen la mateixa arquitectura, requisits i estat real. |
...
| P0-009 | Tancament Escape Room | Fet: definicions Throw/Melody i fills encadenats, porta + balisa al Pipe, prompts Sliding unificats, nomenclatura `New*`/`Cube` eliminada, sales renumerades, sala 11 amb puzles simultàniament visibles i sala 13 amb focus obligatori, botons ▲/▼ compartits PC/VR i autoria dinàmica de 2–8 rodes. Pendent: homogeneïtzar la segona peça de Placement i fer playthrough humà de museu/oficina des de build neta. | Cada puzle té definició, pistes i conseqüència visible; PlayMode supera el mínim. Falta QA humà i tancar Placement. |
