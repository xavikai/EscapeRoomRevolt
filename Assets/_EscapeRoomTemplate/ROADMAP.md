...
|---|---|---|---|
| P0-001 | QA | Fase 1 feta: `EscapeRoomRevolt.Core.asmdef` separat (Save, Settings, Flow, Input, Localització, EventBus/GameContext) amb `EscapeRoomRevolt.Core.Tests.asmdef` referenciant-lo — la suite ja no és 0: 12 tests EditMode reals (`EventBus`, `LocalizationCatalog`) passant al Test Runner. Auditoria prèvia (lectura completa de 145 fitxers a `Core/Runtime`, `Core/Editor`, `Systems/*`, `Player/*`, `UI/*`) va trobar només dues violacions cap a `Core` (`Bootstrapper.cs`, mogut fora de qualsevol asmdef perquè és l'arrel de composició i necessita veure-ho tot; una línia a `SaveManager` cap a `SurvivalDifficultyService`, desacoblada amb un `Func<bool> ManualSaveGate`). Verificat amb Play Mode complet i `Run Framework Smoke Tests`. Pendent: `Systems`/`Player`/`UI` continuen dins `Assembly-CSharp` — l'auditoria hi va trobar entrellaçat real (no un simple moviment de fitxers): `Systems` hi accedeix a tipus concrets `Player.PC`/`Player.VR` en ~12 fitxers en lloc de l'abstracció `IPlayerPlatformAdapter` ja existent, i a `UIManager.Instance`/`UIToolkitMenuController.Instance` directament en ~12 fitxers més en lloc de l'`EventBus` ja existent. Separar-los demana refactoritzar aquests punts, no només crear els `asmdef`. | Mínim 20 EditMode i 10 PlayMode cobrint inventari, combinació, puzles, IDs, perfils, Save/Load, flux de menú i UI crítica; Test Runner amb tests reals i zero errors. **Nota important**: no és independent d'`ARC-001`. Un asmdef de tests no pot referenciar `Assembly-CSharp` (ordre de compilació de Unity), així que cal extreure com a mínim els sistemes provats a un `asmdef` Runtime propi abans d'escriure cap test real. |
...
| ID | Àrea | Implementació pendent | Criteri d'acceptació |
|---|---|---|---|
| ARC-001 | Assemblies | `Core` ja separat (vegeu `P0-001`). Pendent: `Systems`, `Player`, `UI`, `Editor` i XR opcional — requereix trencar les referències de `Systems` cap a tipus concrets de `Player` (usar `IPlayerPlatformAdapter` en lloc de `PlayerMovement`/`VRComfortController` directes) i cap a `UI` (usar `EventBus` en lloc de `UIManager.Instance`/`UIToolkitMenuController.Instance` directes) abans de poder separar-los sense trencar la compilació. | Dependències unidireccionals, XR opcional i temps de recompilació reduït; tests poden referenciar runtime sense internals accidentals. |
