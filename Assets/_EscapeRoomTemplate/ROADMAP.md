...
| ID | Àrea | Implementació pendent | Criteri d'acceptació |
|---|---|---|---|
| ARC-001 | Assemblies | `Core` ja separat (vegeu `P0-001`). Les citacions `Systems`/`Player`→`UI` ja estan desacoblades amb `EventBus` (fase 2b) i la majoria de citacions `Systems`→tipus concrets de `Player` ja passen per `IPlayerPlatformAdapter` (fase 3; `SetMovementBlocked`/`SetLookBlocked`/`TeleportTo`). Queden 4 dependències `Player.PC`/`Player.VR` explícitament acceptades (`TraversalController`, `HidingSpot`, `EvasionController`, `PlayerVisibility`) i `PhysicsSocket`→`VRInteractionBridge` (estat per-objecte, no per-jugador) — vegeu `P0-001` per la justificació de cadascuna. Pendent: `Systems`, `Player`, `UI`, `Editor` i XR opcional encara viuen a `Assembly-CSharp` — cal crear els `asmdef` pròpiament dits (les dependències acceptades que queden són `Systems`→`Player`, una direcció vàlida per a un `asmdef`, no bloquegen la separació). | Dependències unidireccionals, XR opcional i temps de recompilació reduït; tests poden referenciar runtime sense internals accidentals. |
