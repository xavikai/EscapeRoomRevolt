...
| ID | Àrea | Implementació pendent | Criteri d'acceptació |
|---|---|---|---|
| ARC-001 | Assemblies | `Core` ja separat (vegeu `P0-001`). Les citacions `Systems`/`Player`→`UI` ja estan desacoblades amb `EventBus` (fase 2b) i la majoria de citacions `Systems`→tipus concrets de `Player` ja passen per `IPlayerPlatformAdapter` (fase 3; `SetMovementBlocked`/`SetLookBlocked`/`TeleportTo`). `IInteractable`/`InteractionDispatcher` ja són a `Core` (fase 4), resolent la dependència `Player`→`Systems` de `VRInteractionBridge`. Queden 4 dependències `Player.PC`/`Player.VR` explícitament acceptades dins `Systems` (`TraversalController`, `HidingSpot`, `EvasionController`, `PlayerVisibility`) i `PhysicsSocket`→`VRInteractionBridge` (estat per-objecte, no per-jugador) — vegeu `P0-001` per la justificació de cadascuna. Pendent (bloquejador real, no un simple moviment): 3 dependències `Player`→`Systems`/`UI` inverses (`PlayerMovement`→`PlayerVitals`/`AudioManager`, `PlayerInputHandler`→`InventoryManager`, `VRUIToolkitPresenter`→`UIToolkitMenuController`) formen una dependència circular amb les de `Systems`→`Player` — cap `asmdef` de dues bandes ho pot resoldre tal qual; caldrà `EventBus` invertit, promoure algun tipus a `Core`, o acceptar-los explícitament abans de poder crear els `asmdef` de `Systems`/`Player`/`UI`/`Editor` i XR opcional (encara dins `Assembly-CSharp`). | Dependències unidireccionals, XR opcional i temps de recompilació reduït; tests poden referenciar runtime sense internals accidentals. |
